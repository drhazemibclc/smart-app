...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const nodeValue: React.ReactNode = typeof value === 'string' || typeof value === 'number' ? value : null;

    if (labelFormatter) {
      return <div className={cn('font-medium', labelClassName)}>{labelFormatter(nodeValue as string, payload)}</div>;
    }
    ...
    >
      ...
      <div className='grid gap-1.5'>
        {payload.map((item, index) => {
          ...
          const itemLabel: React.ReactNode = itemConfig?.label ?? item.name ?? '';

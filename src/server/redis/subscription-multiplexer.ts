...
const CHANNEL_PREFIX = 'smart-clinic';

/**
 * Helper to create a dedicated Redis client for subscriptions.
 * Multiplexers need their own connection to handle blocking SUBSCRIBE commands.
 */
async function createSubscriberClient(): Promise<Redis> {
  const client = redisManager.getClient().duplicate();
  // ioredis duplicate() doesn't automatically connect if the original wasn't connected
...
export class SubscriptionMultiplexer {
  ...
  private async doInit(): Promise<void> {
    ...
    const patterns = [`${CHANNEL_PREFIX}:*:broadcast:*`, `${CHANNEL_PREFIX}:*:user:${this.userId}:*`];
...
if (!globalForMultiplexer.multiplexerRegistry) {
  globalForMultiplexer.multiplexerRegistry = new Map<string, SubscriptionMultiplexer>();
}

const multiplexerRegistry = globalForMultiplexer.multiplexerRegistry;

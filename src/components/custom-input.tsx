...
interface SwitchProps {
  ...
  setWorkSchedule: React.Dispatch<React.SetStateAction<DaySchedule[]>>;
}

export const SwitchInput = ({ data, setWorkSchedule }: SwitchProps) => {
  // field is keyof Day or boolean true for toggle on/off
  const handleChange = (day: string, field: keyof DaySchedule | true, value?: string) => {
    setWorkSchedule(prevDays => {
      ...
      if (field === true) {
        return [...prevDays, { day: day as Weekday, startTime: '09:00', endTime: '17:00' }];
      }

      // Add new day with specific field update
      return [...prevDays, { day: day as Weekday, startTime: '', endTime: '', [field]: value }];
    });
  };
      ...
      {data?.map(el => (
        ...
        >
          ...
          <div className='hidden items-center gap-2 pl-6 peer-data-[state=checked]:flex'>
            ...
            <Input
              defaultValue='17:00'
              name={`${el.label}.endTime`}
              onChange={e => handleChange(el.value, 'endTime', e.target.value)}
              type='time'
            />
          </div>
        </div>
